...
class attachmentAction extends BackendAction
{
    ...
    protected function _upload_init($upload) {
        ...
        $upload->savePath =  upload_dir('editer/') . $file_type . '/';
    ...
    public function editer_upload() {
        ...
        if ($result['error']) {
            ...
            echo json_encode(array('error'=>0, 'url'=>upload_url('/editer/') . $file_type . '/' . $result['info'][0]['savename']));
        }
        exit;
    }
    ...
    public function editer_manager() {
        $root_path = upload_dir('editer/');
        $root_url = upload_url('/editer/');
