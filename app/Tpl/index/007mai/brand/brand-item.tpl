<div class="show-item show-normal item {$item.class}" id="nid_{$item.num_iid}">
  <div class="show-pic">
    <a biz-itemid="{$item['num_iid']}" isconvert=1 href="" target="_blank" rel="nofollow" onclick="_czc.push(['_trackEvent', '联盟点击', '宝贝', '品牌自动抓取',(new Date().getHours()),'nid_{$item.num_iid}']);">
      <?php if($i < 12 ) { ?>
  ...
  <div class="show-name"><a biz-itemid="{$item['num_iid']}" isconvert=1 target="_blank" href="" rel="nofollow" onclick="_czc.push(['_trackEvent', '联盟点击', '宝贝', '品牌自动抓取',(new Date().getHours()),'nid_{$item.num_iid}']);">{$item.title}</a></div>
