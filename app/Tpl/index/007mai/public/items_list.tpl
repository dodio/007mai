	...
	<ul class="goods-list group">
		...
		<li  class="<eq name="mod" value="3"> last</eq>">
			<div class="sid_{$item.sellerId}  list-good   {$item.class} " id="nid_{$item.num_iid}">
				...
				<h3 class="good-title">
					<a target="_blank" biz-itemid="{$item['num_iid']}" isconvert=1 href="" class="title">{$item.title}</a>
				...
				<div class="good-price">
					...
					<div class="btn-new {$item.class}">
						...
						<elseif condition="($item.class eq 'end') OR ($item.class eq 'sellout') OR ($item.class eq 'buy')"/>
							...
							<a biz-itemid="{$item['num_iid']}" isconvert=1 href=""  target="_blank" rel="nofollow"><em class="icon_n"></em><span>淘宝</span></a>
							<elseif condition="$item.shop_type eq 'B' " />
							<a biz-itemid="{$item['num_iid']}" isconvert=1 href=""  target="_blank" rel="nofollow"><em class="icon_t"></em><span>天猫</span></a>
							<elseif condition="$item.shop_type eq 'D' " />
							<a href="{$item.click_url}" target="_blank" rel="nofollow"><em class="icon_j"></em><span>京东</span></a>
							<else /><a href="" biz-itemid="{$item['num_iid']}" isconvert=1 target="_blank" rel="nofollow"><span>去抢购</span></a>
							</if>
						</if>
					</div>
				</div>
